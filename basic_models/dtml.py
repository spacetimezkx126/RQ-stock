...
class AttentiveLSTM(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_stocks, num_layers=1):
        super(AttentiveLSTM, self).__init__()
        self.hidden_dim = hidden_dim
        self.feature_transform = nn.Linear(input_dim, hidden_dim)
        self.lstm = nn.LSTM(hidden_dim, hidden_dim, num_layers, batch_first=True)
        self.attention = Attention(hidden_dim)  
        self.context_norm = ContextNormalization(num_stocks, hidden_dim)
    ...
    def forward(self, x):
        ...
        context_vector = context_vector.view(batch_size, num_stocks, self.hidden_dim)
...
class DTML(nn.Module):
    def __init__(self, beta_hyp, global_context_index, input_dim, hidden_dim, num_stocks, num_heads, num_layers):
        ...
        self.stock_lstm_model = AttentiveLSTM(input_dim, hidden_dim, num_stocks, num_layers)
        
        # Direct use of DataAxisSelfAttention
        self.self_attention = DataAxisSelfAttention(hidden_dim, num_heads)  # Assuming a single head of attention
        self.final_linear = nn.Linear(hidden_dim, 1)  # Linear layer for prediction
